 ...
 *   - Auto-Confirmation: สามารถตั้งค่าให้ข้ามขั้นตอนการยืนยันรันคำสั่งผ่าน `AUTO_CONFIRM_CMD=true` ได้
 *     เพื่อความสะดวกในการใช้งาน แต่ควรใช้ด้วยความระมัดระวัง
...
// กำหนดว่าจะให้ยืนยันการรันคำสั่งทุกครั้งหรือไม่
const AUTO_CONFIRM_CMD = process.env.AUTO_CONFIRM_CMD === 'true' || process.env.AUTO_CONFIRM_CMD === '1';
...
if (IS_DEBUG_MODE) {
  ...
  debugLog(`Auto-confirm commands: ${AUTO_CONFIRM_CMD}`);
}
    ...
    try {
      ...
      while (!isInteractionDone) {
        ...
        if (commandMatch) {
          ...
          // ตรวจสอบ env variable ว่าต้องการให้ถามหรือไม่
          if (AUTO_CONFIRM_CMD) {
            console.log(chalk.yellow('👉 ได้รับอนุญาตให้รันคำสั่งอัตโนมัติ (AUTO_CONFIRM_CMD=true)'));
            allowExecution = true;
          } else {
            ...
            rl.pause();
            allowExecution = isAffirmative(confirmation);
          }
